...
public class ManagingData {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ManagingData.class.getName());
  ...
  public void removeStudentFromCourse(Connection connection, int studentId, String courseName)
      throws SQLException {
    // Get the course ID based on the course name
    int courseId = getCourseIdByName(connection, courseName);
    ...
    if (courseId != -1) {
      ...
      try (PreparedStatement statement = connection.prepareStatement(sql)) {
        ...
        if (rowsAffected > 0) {
          ...
          System.out
              .println("No enrollment found for student with ID " + studentId + " in course with ID " + courseId + ".");
        }
      }
    } else {
      logger.info("Course '" + courseName + "' not found.");
